...
detail.value = { ...detail.value, workItem: { ...work, workItemId: 'procurement-work', activeRunId: 'procurement-run',
  activeExecutionTarget: 'PROCUREMENT_SOURCING', controlState: 'CLOSED', executionState: 'COMPLETED', outcome: 'ANSWERED' } }
conversation.prepareWork('procurement-work', 'procurement-run', false)
assert.equal(conversation.applyPersisted([{ messageId: 'saved', role: 'ASSISTANT', runId: 'procurement-run',
  sequence: 1, content: '采购方案已保存。', createdAt: now }], 'procurement-run'), true)
assert.equal(conversation.answerState.value, 'COMPLETED')
assert.equal(conversation.entries.value.filter(item => item.type === 'FINAL_ANSWER').length, 1)
