...
export const apiCatalog: ApiEndpointDefinition[] = [
  ...
  { id: 'agent-pause', module: 'Agent', method: 'POST', path: '/api/agent/runs/{runId}/pause', examplePath: '/api/agent/runs/替换为真实-runId/pause', title: '暂停 Run', description: '请求 Runtime 在持久化安全检查点暂停，之后可用相同 runId 恢复。', sideEffect: true },
  ...
  { id: 'agent-stream-events', module: 'Agent', method: 'POST', path: '/api/agent/runs', title: '结构化事件流', description: '使用 Accept: text/event-stream 执行 Agent，并接收包含真实 MODEL_DELTA 的结构化 SSE。', body: { conversationId: 'lab-stream', userId: 'lab-user', question: '解释 Agent Loop', metadata: {} }, sideEffect: true, stream: true },
  { id: 'agent-conversation-messages', module: 'Agent', method: 'GET', path: '/api/agent/conversations/{conversationId}/messages', examplePath: '/api/agent/conversations/替换为真实-conversationId/messages', title: '会话可见消息', description: '读取聊天窗口中的用户与助手消息，不暴露系统提示和工具载荷。', query: { limit: 200 } },
