...
class PublicProcurementRunTests {
    ...
    private final AgentScenarioProfileResolver resolver = new AgentScenarioProfileResolver(List.of(
            new ProcurementSourcingExecutionProfileFactory(), new GeneralAgentExecutionProfileFactory(new AgentProperties())));
    ...
    void blankAndExplicitProcurementUseRealAdaptersAndTrustedCase(Endpoint endpoint) {
        for (String scenario : List.of("", PROFILE)) {
            post(endpoint, scenario, "你好", Map.of("executionTarget", "GENERAL_AGENT", "scenarioId", "general-agent-v1",
    ...
    @Test
    void historyAndResumeBypassNewRunPolicyWithoutProfileConversion() {
        when(runs.recent(20)).thenReturn(List.of());
        ...
        when(runtime.resume("old-run")).thenReturn(new AgentRuntimeResult("old-run", "legacy-session",
                AgentRunState.COMPLETED, AgentStopReason.COMPLETED, "legacy answer", "", null, List.of()));
        client.post().uri("/api/agent/runs/old-run/resume").exchange().expectStatus().isOk()
                .expectBody().jsonPath("$.data.answer").isEqualTo("legacy answer");
        ...
        verify(runtime).resume("old-run");
        verify(runtime).resume(eq("old-run"), any());
        verifyNoInteractions(principals, store, limits);
        verifyNoMoreInteractions(runtime);
    }
