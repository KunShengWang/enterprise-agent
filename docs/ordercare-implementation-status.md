...
| M1 只读智能诊断 | `NOT_STARTED` | 仅有目标设计，没有业务代码 | 案例聚合接口、Profile、inspect 工具、6 类诊断、首批 Eval |
...
- [ ] FlowOrder 定义稳定的 Case DTO，不返回 Entity。
- [ ] 按 `requestId` 聚合预约、扣减、库存不变量和关联死信。
- [ ] 输出枚举化诊断事实和候选动作，不让 Agent 猜业务状态。
- [ ] enterprise-agent 增加可关闭的 FlowOrder typed client。
- [ ] 增加 `ordercare-floworder-v1` 服务端 Profile。
- [ ] 只暴露 `floworder_case_inspect` 与 SOP 检索能力。
- [ ] 至少 6 类诊断分支及首批确定性 Eval。
- [ ] 从现有统一 Run/SSE 窗口完成一次只读诊断。
